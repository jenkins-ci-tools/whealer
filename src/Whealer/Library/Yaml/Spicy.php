<?php
namespace Whealer\Library\Yaml;
...
class Spicy
{
    ...
    private static function _inlineEscape($val)
    {
        ...
        while ( TRUE ) {
            ...
            $finished = TRUE;
            foreach ( $exp as $k => $v ) {
                if ( strpos($v, '__SPICYSEQUENCE__') !== FALSE || strpos($v, '__SPICYMAP__') !== FALSE || strpos($v, '__SPICYSTRING__') !== FALSE ) {
                    $finished = FALSE;
                    break;
                }
            }
